...
\begin{longtable}[c]{@{}p{0.33\textwidth}p{0.67\textwidth}@{}}
...
Flash Attention
(\texttt{OLLAMA\_FLASH\_ATTENTION=1})		& $\sim$10--15\% faster inference.\\
...
and \texttt{/health}. It runs on a Kaggle T4 GPU and is reached from the app over an
...
\begin{tikzpicture}[node distance=12mm and 20mm]
  ...
  \node[block,right=of app] (api) {FastAPI (Kaggle T4)};
...
\end{tikzpicture}
\caption{Deployment path: the app reaches the FastAPI RAG service (on a Kaggle T4 GPU)
through an ngrok tunnel; the service retrieves from ChromaDB and prompts a local Gemma
model.}
\label{fig:deploy}
\end{figure}
...
On an extended set of ten further questions, the optimized pipeline averaged about
$10.9$\,s per answer at roughly $514$ characters, with eight of ten responses rated
excellent, one truncated (a long Arabic answer hitting the camera token limit), and one
minor error. The enriched-embedding fix was decisive for structured queries: an Arabic
``limestone statues'' query that previously returned ``I don't have that info'' now
correctly returns the Double Statue of Ramesses~II, and an English ``Ramesses~II
artifacts'' query expanded from a truncated stub to a list of specific artifacts with
