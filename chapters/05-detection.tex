...
the second shows how the exported model's end-to-end NMS output is parsed on-device.
...
\begin{algorithm}[htbp]
\caption{End-to-end NMS detection parsing (on-device)}
\KwIn{model output $T$ of shape \texttt{[1,300,6]}; per-class thresholds $\tau(\cdot)$}
\KwOut{tracked detections $D$}
...
\ForEach{row $r=(x_1,y_1,x_2,y_2,\mathit{conf},\mathit{cls}) \in T$}{
  \If{$\mathit{conf} \geq \tau(\mathit{cls})$ \textbf{and} box geometry is valid}{
    add $(x_1,y_1,x_2,y_2,\mathit{cls},\mathit{conf})$ to $D$ \tcp*{NMS already applied in-graph}
  }
}
\Return $D$\;
\end{algorithm}
...
The v2 validation metrics \emph{overstate} real-world generalization through
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{End-to-end NMS format.} The exported output has shape
  ...
  \item \textbf{Footprint.} The deployed asset is FP16 TFLite at \textbf{5.97\,MB},
        running in roughly $800$\,ms per frame on a mid-tier Android phone corresponding to a detection update rate of about 1.25 FPS.
        Although this falls below initial target detection rates, the overall user experience remained smooth because inference was executed asynchronously on a separate thread,
        allowing the camera preview and user interface to remain responsive and non-blocking throughout operation.
\end{itemize}

% \section{Limitations}

% The headline validation metrics aren't bounded by frame leakage. Deployment-facing numbers are the held-out test figures
...
% \TimeLens{} system.
For fine-grained museum artifact detection, \textbf{label quality is a first-order factor}, while model architecture and input resolution are secondary considerations. A compact YOLOv8n model trained on clean, hand-verified annotations demonstrated excellent real-world performance during deployment and field testing. Across all artifact classes included in the dataset, the model produced no observed misclassifications, with detection confidence scores consistently ranging between 75\% and 95\%. These results indicate that the detector is sufficiently accurate and reliable for practical museum deployment as part of the TimeLens system.
