% !TEX TS-program = lualatex
% =====================================================================
%  TimeLens: On-Device Detection and Retrieval-Grounded Bilingual Q&A for the Grand Egyptian Museum
%  Graduation Project Thesis -- Capital University, FCAI
%  Build:  latexmk main.tex           (XeLaTeX + BibTeX; engine set in .latexmkrc)
%  If IEEEtran.bst is missing, change \bibliographystyle below to plainnat.
% =====================================================================
\documentclass[12pt,a4paper,oneside]{report}

\input{preamble}

\begin{document}

% ---------------------------------------------------------------- front
\pagenumbering{roman}             % title page = "i" (hidden); avoids a duplicate
\input{frontmatter/titlepage}     % arabic "page.1" anchor colliding with Chapter 1
\setcounter{page}{2}              % front matter proper then continues at "ii"

\input{frontmatter/declaration}
\input{frontmatter/abstract}
\input{frontmatter/graphical_abstract}
\input{frontmatter/acknowledgment}
\input{frontmatter/dedication}

\tableofcontents
\listoffigures
\listoftables
\listofalgorithms
\input{frontmatter/list_of_equations}
\input{frontmatter/abbreviations}
\input{frontmatter/glossary}
\input{frontmatter/list_of_publications}

% ---------------------------------------------------------------- main
\cleardoublepage
\pagenumbering{arabic}

\input{chapters/01-introduction}
\input{chapters/02-literature-review}
% \input{chapters/03-requirements}
\input{chapters/03-analysis-and-design}
% \input{chapters/04-architecture}
\input{chapters/05-detection}
\input{chapters/06-rag}
\input{chapters/07-market}
\input{chapters/08-conclusion}

% ---------------------------------------------------------------- back
\appendix
\input{appendices/a-ethics}
\input{appendices/b-reproducibility}
\input{appendices/c-genai-disclosure}

\cleardoublepage
\bibliographystyle{IEEEtran}   % fallback: plainnat
\bibliography{references}

\cleardoublepage
\input{frontmatter/abstract_arabic}
\input{frontmatter/titlepage_arabic}
\end{document}

